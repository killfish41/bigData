...
public class test02 {
    public static void main(String[] args) throws Exception {
        BufferedReader bf = new BufferedReader(new FileReader("H:\\安装资料\\任务\\任务\\zpsjj\\jobs2.csv"));
        BufferedWriter bw = new BufferedWriter(new FileWriter("H:\\安装资料\\任务\\任务\\zpsjj\\jobs2-processed.csv"));
        String s = null;
        String tmp ="";
        int num = 0;
        boolean flag = false;
...
//            System.out.println(s.split(",").length);
            if(s.split(",").length==14||tmp.split(",").length==14){
                if (flag&&tmp.split(",").length==14){
                    ...
                    String line = strs[0]+","+strs[1]+","+strs[2]+","+strs[3]+","+strs[4]+","+strs[5]+","+people_min+","+people_max+","+strs[7]+","+experience_min+","+experience_max+","+strs[9]+","+strs[10]+","+salary_min+","+salary_max+","+strs[12]+","+strs[13];
                    bw.write(line);
                    bw.newLine();
